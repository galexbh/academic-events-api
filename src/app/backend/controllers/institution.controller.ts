...
import { createInstitution, findInstitutions } from '../services/institution.services';

export class InstitutionController {

    public async createInstitutionHandler(req: Request<{}, {}, CreateCategoryInput>, res: Response) {
    ...
    public async getAllInstitutionHandler(_req: Request, res: Response) {
        try {
            ...
            return res.status(StatusCodes.OK).json({ message: "Institutions found", Institutions });
        } catch (e: any) {
            return res.status(StatusCodes.NOT_FOUND).json({ message: "No institutions found" });
        }
    }
}